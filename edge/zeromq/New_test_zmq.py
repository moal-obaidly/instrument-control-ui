...
def start_signal():
    global running, rtt_thread, buffer_thread, signal_thread, seq_num, checksum, count
    running = True
    ...
    def signal_loop():
        global running, seq_num, checksum, count
        BATCH_SIZE = 512
        print(f"Flooding {BATCH_SIZE} samples/batch...")
        while running:
            batch = []
            for _ in range(BATCH_SIZE):
                adc_value = 123.456  # Simulated value; change if needed
