...
class MQTTClient:
        ...
        if msg.topic == "experiment/data":
            try:                                                  #
                ...
                if now - self.last_log_time >= 5:
                    throughput_bps = self.bytes_received / (now - self.last_log_time)
                    print(f"[{datetime.now().isoformat()}] Throughput: {throughput_bps / 1000:.2f} KB/s")
                    self.throughput_log.append((datetime.now(), throughput_bps))
                    self.bytes_received = 0
                ...
                #timerec = int(time.time()*1000.0)
                    if seq not in self.received_seqs:
                        ...
                        if self.current_time - self.old_time >= 1.0:
    ...
    def start(self):
        ...
        self.client.connect("100.106.113.72", 1883, 60) #.36 for laptop, .82 for rpi4   taliscale: 100.106.113.72
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.throughput_label = QtWidgets.QLabel("Throughput: -- KB/s")
        ...
        main_layout.addWidget(self.throughput_label)
    ...
    def stop_experiment(self):
        ...
        print(f"RTT average: {self.mqtt_client.rtt_total/ self.mqtt_client.rtt_counter}")

        with open("throughput_log.csv", "w") as f:
...
# Different possible simulated sampling rates
    ...
    def toggle_layout(self):
        ...
        if not self.dark_mode_enabled:
            ...
            self.dark_mode_enabled = False
        print(f"delay:{self.mqtt_client.current_time-self.stop_time}")
