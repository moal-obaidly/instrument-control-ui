                ...
                try:
                    ...
                    if topic == b"experiment/data":
                        ...
                        if now - self.last_log_time >= 5:
                            ...
                            self.throughput_log.append((time.time(), throughput_bps))
                            self.bytes_received = 0
...
class MainWindow(QWidget):
        ...
        while True:
            try:
                ...
                if topic == "experiment/rtt":
                    ...
                    self.rtt_total += self.rtt_client.rtt 
                    self.rtt_counter += 1
    ...
    def stop_experiment(self):
        ...
        print(f"RTT average: {self.rtt_total/ self.rtt_counter}")

        with open("throughput_log_zmq.csv", "w") as f:
            f.write("timestamp,throughput_bps\n")
            for ts, tput in self.zmq_sub_client.throughput_log:
                f.write(f"{datetime.fromtimestamp(ts).isoformat()},{tput}\n")
